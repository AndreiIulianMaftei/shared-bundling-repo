...
def count_bundles_coarse(img_bgr: np.ndarray,
    ...
    bundle_count = G.number_of_edges()

    return bundle_count, thicknesses, edges
...
def map_bundles_to_graph(G, shape, edges, scale = 0.25,):
    ...
    ammap = np.zeros((int(w)+1, int(h)+1), dtype=np.uint16)
    
    ambiguity = {}
    
    for i, edge in enumerate(edges):
        for y,x in edge:
            if 0 <= int(x) < w and 0 <= h - int(y) < h:
